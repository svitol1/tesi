"""
================================================================================
Script di testing e valutazione - MLP
================================================================================

Esegue l'inferenza sui segmenti ECG e applica un'aggregazione a livello
di registrazione (con soglia >50%).

Compatibile con il test set prodotto da dataset_builder.py
(cartella test/, con beats_index.csv contenente TUTTI i battiti validi
di ogni registrazione, etichetta "normal" per tutte).

Identico, nella logica di caricamento dati e aggregazione, a test.py
(la versione per ECG_CNN): l'unica differenza è il modello caricato
(ECG_MLP al posto di ECG_CNN) e i relativi iperparametri costruttivi
(hidden_dims al posto di hidden_channels, seq_len esplicito perché l'MLP,
a differenza della CNN, non è indipendente dalla lunghezza della finestra
- vedi mlp_model.py). Il preprocessing (z-score per derivazione) resta lo
stesso, tramite ECGSegmentDataset importato da mlp_dataset.py.

Uso da riga di comando:
    python test_mlp.py --data_dir data_prep/combined_dataset/test --weights ecg_mlp_weights.pt \
...
try:
    ...
    # Allow running this file directly: python test/test_mlp.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Testing ECG_MLP per rilevazione malposizionamento elettrodi"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Elenco classi nell'ORDINE usato in training (stesso ordine con cui "
             "train_mlp.py le ha enumerate da sorted(df['label'].unique()) sul train set). "
...
def main():
    ...
    # train_mlp.py costruisce label_to_idx da sorted(df["label"].unique()) sul
    ...
    # NB: hidden_dims, dropout e use_batchnorm devono corrispondere a quelli
    # usati in training (vedi train_mlp.py / best_hparams_mlp.txt se generato
    # da train_mlp_optuna.py), altrimenti load_state_dict fallisce per shape
    # mismatch. seq_len deve combaciare con SEGMENT_LEN sopra, per lo stesso
    # motivo spiegato in mlp_model.py (l'MLP non è indipendente da T).
    ...
    output_csv = "test_mlp_recordings_summary.csv"
